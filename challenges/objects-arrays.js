...
const unisWithUni = graduates.filter(
  student => student.university.search("Uni") > -1
);
...
zooAnimals.forEach(animal => {
  const animalString = `Name: ${animal.animal_name}, Scientific: ${animal.scientific_name}.`;
  displayNames.push(animalString);
});
...
The zoos are concerned about animals with a lower population count. Using filter, create a new array of objects called lowPopulationAnimals which contains 
...
const populationTotal = zooAnimals.reduce(
  (population, animal) => population + animal.population,
  0
);
